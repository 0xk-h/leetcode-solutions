...
- uses bitmasking to keep track of seen column and diagonals and n-way backtracking
- the diagonals value can be in negative so added n so that the range is always `0 to 2*n`
- **Time Complexity** -> $O(n!)$
...
- standard use of set to keep track of seen columns and diagonals
- **Time Complexity** -> $O(n!)$
- **Space Complexity** -> $O(n^2)$
